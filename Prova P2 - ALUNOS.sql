...
-- Nome do aluno: Pedro Felipe Batista Sousa da Silva
...
CREATE DATABASE LOJAMARIACJ3028721;
GO


-- Habilita o contexto do banco de dados
USE LOJAMARIACJ3028721;
GO
...
SELECT NumeroPedido AS "Número do Pedido",
       COUNT(DISTINCT CodigoProduto) AS "Quantidade de Produtos Diferentes",
       SUM(Quantidade) AS "Total de Itens no Pedido"
FROM item_de_pedido
...
SELECT CodigoVendedor AS "Código do Vendedor",
       NomeVendedor AS "Nome do Vendedor",
       Salario AS "Salário",
       (SELECT AVG(Salario) FROM VENDEDORES) AS "Média Salarial Geral"
FROM 
    vendedores
WHERE
...
SELECT     CodigoCliente      AS "Código do Cliente",
           NomeCliente       AS "Nome do CLiente",
           DescricaoProduto  AS "Produto Comprado",
           ValorUnitario     AS "Valor Unitário",
           Quantidade        AS "Quantidade Comprada",
           (ValorUnitario * Quantidade) AS "Valor Total",
           PrazoEntrega      AS "Prazo de Entrega",
           NomeVendedor      AS "Nome do Vendedor"
FROM
...
DROP DATABASE LOJAMARIACJ3028721;
GO
